const data = {
  banner: {
    heading: 'Startereum',
    subheading: 'A Platform for Startup Co-creation, Curation and Funding.',
    image: 'http://lorempixel.com/600/400/',
    button: {
      telegram: {
        label: 'Join Telegram',
        href: '/telegram'
      },
      whitepaper: {
        label: 'Read whitepaper',
        href: '/white'
      }
    },
    news: {
      text: 'A Platform for Startup Co-creation, Curation and Funding.A Platform for Startup Co-creation, Curation and Funding.A Platform for Startup Co-creation, Curation and Funding.'
    }
  },
  slides: [
    {
      bg: 'light',
      variant: 'rightFloat',
      heading: ['Gameified Consensus  Platform', 'for Value Creation'],
      description:
      ['Startereum is a unique tokenized platform for investment discovery. It allows complex decisions to be made by multiple stakeholders.',
        'Players earn differently based on the teams they discover and support. All token holders win together as the platform&apos;s pool of value grows.'],
      image: '/assets/illustrations/illustration-1.svg'
    },
    {
      bg: '',
      variant: '',
      heading: ['Early Funding and', 'Structured Support'],
      description:
      ['The platform provides early funding to high potential startups in exchange for tokens or equity. It then helps them to grow through a community-driven self-acceleration model.'],
      image: '/assets/illustrations/illustration-2.svg'
    },
    {
      bg: 'light',
      variant: 'rightFloat',
      heading: ['Tech and Strategic Inputs'],
      description:
      ['Teams can use the platform to discover high quality technical consultants and strategic advisors from all around the world.'],
      image: '/assets/illustrations/illustration-3.svg'
    },
    {
      bg: '',
      variant: '',
      heading: ['Thousands of Hands', ' Ready to Help'],
      description:
      ['Discover your early users, supporters, and token buyers from among the thousands of members on the Startereum platform.'],
      image: '/assets/illustrations/illustration-4.svg'
    },
    {
      bg: 'light',
      variant: 'rightFloat',
      heading: ['Follow On Funding', 'by Member Investors'],
      description: ['Teams that win the support and confidence of the community can then achieve seed funding or token pre sales from an in-house network of member investors.'],
      image: '/assets/illustrations/illustration-5.svg'
    }

  ],
  strDescription: {
    heading: 'Startereum Token (STR)',
    description: 'About Startereum Token. Startereum token allows community to participate in creating, curating and funding new startup projects. It also serves as the voting token to guide the evolution of the startereum protocol.',
    image: '../assets/coin.png'
  },
  cards: [
    'A Unique Value Creation Token',
    'Backed by a Growing Asset Pool',
    'Flows Circularly between Platform and Community',
    'Mined through Proof of Human Work',
    '<b>ERC 20</b> Standard on Ethereum'
  ],
  team: [
    {
      heading: 'Team',
      members: [
        {
          name: 'Dr. Aditya Dev Sood',
          designnation: 'Chairman & Founder',
          company: 'Startup Tunnel',
          fb: 'https://www.facebook.com/adityasood',
          twitter: 'https://twitter.com/adityadevsood',
          linkedIn: 'https://www.linkedin.com/in/adityadevsood/',
          img: '/assets/team/aditya-dev-sood.jpg'
        },
        {
          name: 'Saurabh Arora',
          designnation: 'Founder & CEO',
          company: 'Airwoot',
          fb: 'https://www.facebook.com/tanish2k',
          twitter: 'https://twitter.com/tanish2k',
          linkedIn: 'https://www.linkedin.com/in/saurabhairwoot/',
          img: '/assets/team/saurabh-arora.jpg'
        },
        {
          name: 'Sasu Ristimäki',
          designnation: 'Founder',
          company: 'SRI Capital',
          fb: 'https://www.facebook.com/sasu.ristimaki',
          twitter: 'https://twitter.com/SRistimaki',
          linkedIn: 'https://www.linkedin.com/in/sasu-ristim%C3%A4ki-834b7a1/',
          img: '/assets/team/sasu.jpg'
        }
      ]
    },
    {
      heading: 'Advisors',
      members: [
        {
          name: 'Sunil Agarwal',
          designnation: 'Director of Trustmachines',
          company: 'Blockchain Advisor',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/sunil-aggarwal-9a994726/',
          img: '/assets/team/sunil-agarwal.jpg'
        },
        {
          name: 'Sandeep Sangli',
          designnation: 'Co Founder',
          company: 'EasterEgg Pte Ltd',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/sandeepsangli/',
          img: '/assets/team/sandeep-sangli.jpg'
        },
        {
          name: 'Kunal Nandwani',
          designnation: 'Cofounder and CEO',
          company: 'Hashcove Limited',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/kunalnandwani/',
          img: '/assets/team/kunal-nandwani.jpg'
        },
        {
          name: 'Ritesh Modi',
          designnation: 'Architect, Evangelist',
          company: ' and Microsoft Regional Lead - MCT',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/ritesh-modi/',
          img: '/assets/team/ritesh-modi.jpg'
        }
      ]
    },
    {
      heading: 'Community Leaders',
      members: [
        {
          name: 'Kartik Sharma',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/shkartik90/',
          img: '/assets/team/kartik-sharma.jpg'
        },
        {
          name: 'Sangeethasruthi S',
          designnation: 'Financial Planner and Consultant',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/sangeethasruthi-s-68857490/',
          img: '/assets/team/sangeetha-sruthi.jpg'
        }
      ]
    },
    {
      heading: 'Program Partners',
      members: [
        {
          name: 'Vishwanathan Sahasranamam',
          designnation: 'Co-Founder & CEO FORGE',
          company: 'Innovation Accelerator',
          fb: 'https://www.facebook.com/VishwanathIIMB',
          twitter: 'https://twitter.com/VishwanathForge',
          linkedIn: 'https://www.linkedin.com/in/vishwanathforge/',
          img: '/assets/team/vishwanath.jpg'
        },
        {
          name: 'Abhishek Sanghvi',
          designnation: 'Co Founder',
          company: 'Swan Angel Fund',
          fb: 'https://www.facebook.com/abhiworld',
          twitter: 'https://twitter.com/sheksphere',
          linkedIn: 'https://www.linkedin.com/in/abhisheksanghvi/',
          img: '/assets/team/abhishek-sanghvi.jpg'
        },
        {
          name: 'Amit Singh',
          designnation: 'MD Headstart Network ',
          company: 'Foundation',
          fb: 'https://www.facebook.com/abeoye',
          twitter: 'https://twitter.com/AbeOye',
          linkedIn: 'https://www.linkedin.com/in/startupamit/',
          img: '/assets/team/amit-singh.jpg'
        }

      ]
    },
    {
      heading: 'Mentors',
      members: [
        {
          name: 'Sameer Gupta',
          designnation: 'Director Future First Group FZ',
          company: 'LLC',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/sameer-gupta-a86a8410/',
          img: '/assets/team/sameer-gupta.jpg'
        },
        {
          name: 'Jay Kannaian',
          designnation: 'CEO & Co-founder',
          company: 'Smart Air Filter',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/jaykannaiyan/',
          img: '/assets/team/jay-kannaiyan.jpg'
        },
        {
          name: 'Nalin Agarwal',
          designnation: 'India Catalyst',
          company: 'New Energy Nexus',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/nalinag/',
          img: '/assets/team/naleen-agarwal.jpg'
        },
        {
          name: 'Namrata Mehta',
          designnation: 'Mentor & Independent',
          company: 'Innovation Consultant',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/namrata-mehta-53696640/',
          img: '/assets/team/namrata-mehta.jpg'
        },
        {
          name: 'Bhaskar Ghosh',
          designnation: 'VP of Engineering & Operations',
          company: 'NerdWallet',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/ghoshbhaskar/',
          img: '/assets/team/bhaskar-ghosh.jpg'
        },
        {
          name: 'Mrutunjay Mishra',
          designnation: 'Co-Founder',
          company: 'Juxt Smart Mandate',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/mrutyunjay/',
          img: '/assets/team/mritunjay-mishra.jpg'
        },
        {
          name: 'Rakesh Mishra',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: '',
          img: '/assets/team/rakesh-mishra.jpg'
        },
        {
          name: 'Arpit Agarwal',
          designnation: 'Principal',
          company: 'Blume Ventures',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/arpit105/',
          img: '/assets/team/arpit-agarwal.jpeg'
        },
        {
          name: 'Prajakt Raut',
          designnation: 'Founding Partner',
          company: 'The Growth Labs',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/prajaktraut/',
          img: '/assets/team/prajakt-raut.jpeg'
        },
        {
          name: 'Dipesh Garg',
          designnation: 'Founder',
          company: 'Truelancer.com',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/dipeshgarg/',
          img: '/assets/team/dipesh-garg.jpg'
        },
        {
          name: 'Jatin Modi',
          designnation: 'CEO',
          company: 'FrogIdeas',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/jatinmodi/',
          img: '/assets/team/jatin-modi.jpg'
        },
        {
          name: 'Apurva Chamaria',
          designnation: 'Vice-President & Head',
          company: 'Corporate Marketing, HCL Tech',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/apurvachamaria/',
          img: '/assets/team/apurva-chamaria.jpg'
        },
        {
          name: 'Nischal Dua',
          designnation: 'Founder',
          company: 'The Remote Life',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/nishchaldua/',
          img: '/assets/team/nischal-dua.jpg'
        },
        {
          name: 'Ashish Mehra',
          designnation: 'Senior Manager',
          company: 'NetApp',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/drashishmehra/',
          img: '/assets/team/ashish-mehra.jpg'
        },
        {
          name: 'Amit Pande',
          designnation: 'Senior Director, Product',
          company: 'Marketing at Tact',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/amitpande/',
          img: '/assets/team/amit-pande.jpg'
        },
        {
          name: 'Anjali Krishnan',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: '',
          img: '/assets/team/anjali-krishnan.jpg'
        },
        {
          name: 'Nishant Jain',
          designnation: 'CEO',
          company: 'Design For Use',
          fb: '',
          twitter: '',
          linkedIn: 'https://www.linkedin.com/in/nishj/',
          img: '/assets/team/nishant-jain.jpg'
        },
        {
          name: 'Sandeepan Banerjee',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: '',
          img: '/assets/team/sandeep-banerjee.jpg'
        },
        {
          name: 'Miten Mehta',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: '',
          img: '/assets/team/miten-mehta.jpg'
        },
        {
          name: 'Kabir Kumar',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: '',
          img: '/assets/team/kabeer.jpg'
        },
        {
          name: 'Sandy Sharma',
          designnation: '',
          company: '',
          fb: '',
          twitter: '',
          linkedIn: '',
          img: '/assets/team/sandy.jpg'
        }
      ]
    }
  ]
};

export {data};
